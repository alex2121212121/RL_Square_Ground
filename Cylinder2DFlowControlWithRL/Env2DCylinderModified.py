...
class Env2DCylinderModified(gym.Env):
    ...
    def step(self, actions=None):
        ...
        if("single_run" in self.inspection_params and self.inspection_params["single_run"] == True):
            self.save_PSR()
            # Only valid for 'power_reward' currently
    ...
    def save_state(self, next_state):
        ...
        if (not os.path.exists("saved_models/" + name)):
            with open("saved_models/" + name, "w") as csv_state:
                ...
                for v in range(len(next_state)):
                    spam_writer.writerow([self.episode_number, self.solver_step, next_state[v]])
        else:
    ...
    def save_PSR(self):
        ...
        if (not os.path.exists("saved_models/" + name)):
            with open("saved_models/" + name, "w") as csv_file:
                spam_writer = csv.writer(csv_file, delimiter=";", lineterminator="\n")
                spam_writer.writerow(["Episode", "Step", "PSR"])
                spam_writer.writerow([self.episode_number, self.solver_step-1, self.PSR])
        else:
            with open("saved_models/" + name, "a") as csv_file:
                spam_writer = csv.writer(csv_file, delimiter=";", lineterminator="\n")
                spam_writer.writerow([self.episode_number, self.solver_step-1, self.PSR])
